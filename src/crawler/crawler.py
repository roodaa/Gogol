...
class Crawler:
    ...
    def __init__(self, max_pages: Optional[int] = None):
        ...
        self.max_pages = max_pages if max_pages is not None else CRAWLER_CONFIG["max_pages"]
    ...
    def _is_wikipedia_url(self, url: str) -> bool:
        """
        Vérifie si l'URL est une page Wikipedia.
        
        Args:
            url: URL à vérifier
        
        Returns:
            True si c'est une page Wikipedia
        """
        parsed = urlparse(url)
    ...
    def _extract_wikipedia_content(self, soup: BeautifulSoup) -> str:
        """
        Extrait UNIQUEMENT le contenu principal d'une page Wikipedia.
        
        Évite d'indexer les éléments communs à toutes les pages:
        - Menu de navigation
        - Header
        - Footer
        - Sidebar
        ...
        Args:
            soup: BeautifulSoup object de la page Wikipedia
        
        Returns:
            Texte du contenu principal uniquement
        """
        # Stratégie 1: Chercher le contenu principal (div#mw-content-text)
        main_content = soup.find('div', id='mw-content-text')
        
        # Stratégie 2: Si pas trouvé, chercher bodyContent
        if not main_content:
            main_content = soup.find('div', id='bodyContent')
        
        # Stratégie 3: Si toujours pas trouvé, chercher div#content
        if not main_content:
            main_content = soup.find('div', id='content')
        
        # Fallback
        if not main_content:
            self.logger.warning("Structure Wikipedia non reconnue, extraction complète")
            main_content = soup
        
        # Éléments à supprimer
        elements_to_remove = [
            'script', 'style', 'noscript',
            '.navbox', '.infobox', '.ambox',
            '.mw-editsection', '#toc', '.reflist',
        ...
        for selector in elements_to_remove:
            if not selector.startswith(('.', '#')):
                ...
                for element in content_copy.find_all(class_=selector[1:]):
                    element.decompose()
            else:
                element = content_copy.find(id=selector[1:])
                if element:
                    element.decompose()
    ...
    def _parse_page(self, response: requests.Response, url: str) -> Dict:
        ...
        # ⚠️ MODIFICATION: Extraction intelligente selon le type de site
        if self._is_wikipedia_url(url):
            # Wikipedia: extraire UNIQUEMENT le contenu principal
            self.logger.debug(f"Page Wikipedia détectée: extraction du contenu principal")
            text = self._extract_wikipedia_content(soup)
        else:
            # Autre site: comportement normal
            self.logger.debug(f"Site standard: extraction complète")
